...
class List {
  ...
  async getList(req, res, next) {
    try {
      const list = await conn.queryPromise('SELECT * FROM list ORDER BY index_order');
      const addedMyDayTodo = await conn.queryPromise('SELECT count(id) AS addede_my_day_todo_count FROM todo WHERE todo.added_my_day = 1 AND todo.marked_as_completed=0');
      const markedAsImportantTodo = await conn.queryPromise('SELECT count(id) AS marked_as_important_todo_count FROM todo WHERE todo.marked_as_important = 1 AND todo.marked_as_completed=0');
      const plannedTodo = await conn.queryPromise('SELECT count(id) AS planned_todo_count FROM todo WHERE todo.due_date IS NOT NULL AND todo.marked_as_completed=0');
      ...
      const numberOfTodoInListId = await conn.queryPromise('SELECT list_id, count(list_id) FROM todo WHERE todo.marked_as_completed=0 GROUP BY list_id');
  ...
  async addList(req, res, next) {
    ...
    try {
      ...
      await conn.queryPromise(`INSERT INTO list (name, index_order) VALUES (?, ?)`, [req.body.name, orderResults[0].max_index_order + 1]);
      res.send({
        code: 200,
        message: 'success',
      });
