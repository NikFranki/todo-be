...
class Todo {
  ...
  async getList(req, res, next) {
    ...
    // SELECT todo.id, content, list_id, note, category FROM todo, list WHERE todo.list_id = list.id;
    ...
    const sqlStatus = `${sqlTotal}${idQuery}${listIdQuery}${contentQuery} ORDER BY todo.create_time DESC`;
    const values = [];
    if (idQuery) {
      values.push(id);
    }
