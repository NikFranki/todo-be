...
class Todo {
  constructor() {
    ...
    const job = schedule.scheduleJob(rule, () => {
      ...
      this.reminderTodos.forEach(item => {
        if (item.reminder_time_alias === currentTime) {
  ...
  async getReminderTodo() {
    const list = await conn.queryPromise('SELECT *, DATE_FORMAT(reminder, "%Y-%m-%d %H:%i:%s") AS reminder_time_alias FROM todo WHERE reminder IS NOT NULL', []);
  ...
  async updateTodo(req, res, next) {
    const { content, list_id, added_my_day = 0, marked_as_important = 0, marked_as_completed = 0, due_date, reminder, note, category, id } = req.body;
