...
const { MY_DAY, IMPORTANT, DEFAULT_LIST_IDS, MARKED_AS_IMPORTANT, ADDED_MY_DAY } = require('../constant');
...
class Todo {
  ...
  async getList(req, res, next) {
    ...
    const sqlStatus = `${sqlTotal}${idQuery}${listIdQuery}${addedMyDayQuery}${markedAsImportantQuery}${contentQuery} ORDER BY todo.create_time DESC`;
    const values = [];
    if (idQuery) {
      values.push(id);
    }
