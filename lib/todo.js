...
class Todo {
  ...
  async getList(req, res, next) {
    ...
    const sqlTotal = 'SELECT todo.id, content, status, list_id, note, category, date FROM todo, list';
    ...
    const listQuery = ` ${idQuery || statusQuery || contentQuery ? 'AND' : 'WHERE'} todo.list_id = list.id`;
    const paginationQuery = pageSize && pageNo ? ` LIMIT ${pageSize} OFFSET ${pageSize * (pageNo - 1)}` : '';
    const sqlStatus = `${sqlTotal}${idQuery}${statusQuery}${contentQuery}${listQuery} ORDER BY todo.create_time DESC`;
