...
class Todo {
  ...
  async getList(req, res, next) {
    ...
    const sqlTotal = 'SELECT * FROM todo';
    ...
    const dueDateQuery = list_id === PLANNED ? ` ${idQuery ? 'AND' : 'WHERE'} todo.due_date IS NOT NULL` : '';
    const contentQuery = content ? ` WHERE content LIKE '${content}%'` : '';
    const sqlStatus = `${sqlTotal}${idQuery}${listIdQuery}${addedMyDayQuery}${markedAsImportantQuery}${dueDateQuery}${contentQuery} ORDER BY todo.create_time DESC`;
    const values = [];
    if (idQuery) {
      values.push(id);
    }
